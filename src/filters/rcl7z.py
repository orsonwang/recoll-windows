#!/usr/bin/env python3
"""7-Zip (.7z) archive filter for Recoll.

This version uses the 7-Zip command-line program (7z) to list and extract
archive members, instead of the py7zr/pylzma python modules. 7z is already
bundled with the Windows distribution, so no python module is needed.
...
member): extracting a member at a time would re-decompress the whole solid
block for every member of a solid .7z, and would buffer each member fully in
memory. Members are then read from disk, and members larger than the configured
maxmembersize are skipped without being read.
"""
...
class SevenZipExtractor(ArchiveExtractor):
    ...
    def _ensure_extracted(self):
        ...
        if self.tmpdir is None:
            self.tmpdir = rclexecm.SafeTmpDir("rcl7z")
